...
@Repository
public class ManualTriggerDaoImpl implements ManualTriggerDao {
	...
	public List<DBObject> getManualTriggers(ManualTrgrsReqModel pRequestModel, String collName) {
		...
		try {
			...
			if (pRequestModel.getFromDate() != null && pRequestModel.getToDate() != null)
				searchQuery.put("dep_date", new BasicDBObject("$gte", pRequestModel.getFromDate()).append("$lte",
			...
			dbObjList = cursor.sort(new BasicDBObject("dep_date", -1)).toArray();
